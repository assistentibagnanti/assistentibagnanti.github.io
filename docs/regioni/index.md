# Le Regioni

**Attenzione: la normativa nazionale è solo il "pavimento". Ogni Regione la completa e può
aggiungere requisiti.** Per sapere cosa vale *davvero* nella tua piscina devi guardare anche la
normativa della **tua Regione**.

## Come funziona (in breve)

1. L'[**Accordo Stato-Regioni del 16 gennaio 2003**](../quadro-normativo/accordo-stato-regioni.md)
   (e il successivo accordo del 16 dicembre 2004) fissa i **principi e i requisiti minimi** validi su
   tutto il territorio nazionale: classificazione delle piscine, dotazione di personale, controlli.
2. Quei principi **non sono direttamente applicabili** finché ogni Regione non li **recepisce** con un
   proprio atto: una **legge regionale**, un **regolamento** o una **delibera di Giunta (DGR)**.
3. Nel recepire, la Regione può **dettagliare e irrigidire** le regole — per esempio il **rapporto tra
   assistenti bagnanti e superficie/affollamento**, i requisiti del responsabile della piscina, gli
   obblighi gestionali.

!!! danger "La conseguenza pratica per l'AB"
    Due piscine identiche in due Regioni diverse possono avere **obblighi diversi** su quanti AB
    servono e quando. La regola che ti riguarda è quella **della tua Regione** (oltre al minimo
    nazionale). In caso di dubbio o contestazione, è lì che devi guardare.

## Come trovare la norma della tua Regione

Questo metodo vale **sempre**, ed è il modo più sicuro di arrivare al testo giusto e aggiornato:

- Cerca: **«[nome Regione] regolamento piscine»** oppure **«[nome Regione] recepimento accordo
  Stato-Regioni piscine»**.
- Guarda sul sito ufficiale dell'**Assessorato alla Sanità** della tua Regione o dell'**ARPA/ASL**.
- Chiedi all'**ASL** competente o al **Comune**: sono tenuti a indicarti la norma applicabile.
- Verifica sempre sulla **fonte ufficiale** (Bollettino Ufficiale Regionale – BUR) che sia la
  versione **vigente**.

## Riferimenti per Regione

!!! note "Una precisazione importante"
    Contrariamente a quel che si potrebbe pensare, **quasi tutte le Regioni hanno un proprio atto
    di recepimento** (spesso una **delibera di Giunta**, a volte una legge regionale).
    Fanno eccezione **Sicilia** e **Valle d'Aosta**, che non risultano aver adottato un proprio
    provvedimento sulle piscine: lì si applica direttamente il quadro nazionale (Accordo 2003 +
    Disciplina interregionale 2004). Per la Valle d'Aosta lo conferma la stessa **Azienda USL**, che
    nella vigilanza igienico-sanitaria sulle piscine richiama soltanto l'Accordo Stato-Regioni 2003.
    Capire se un recepimento **aggiunge** regole oppure no richiede leggere il singolo atto: la
    colonna **«Tipo»** qui sotto serve da prima indicazione.

**Come leggere la colonna «Tipo»:**

- **Legge / Regolamento regionale** → la Regione ha una **disciplina propria** con regole
  aggiuntive o specifiche: **leggila**, può cambiare gli obblighi sugli AB.
- **Delibera di recepimento** → in genere recepisce il quadro nazionale; **alcune** introducono
  però adattamenti propri (es. la Lombardia ha requisiti specifici sugli AB): **verifica**.
- **— (solo normativa nazionale)** → la Regione non ha un proprio atto: vale direttamente il
  quadro nazionale (Accordo 2003 + Disciplina interregionale 2004). È il caso di **Sicilia** e
  **Valle d'Aosta**.
- *da verificare* → atto non ancora confermato da noi.

| Regione | Atto di recepimento | Tipo |
|---|---|---|
| Abruzzo | D.G.R. 5 aprile 2004, n. 206 | Delibera |
| Basilicata | L.R. 9 luglio 2020, n. 19 | Legge regionale |
| Calabria | D.G.R. 12 dicembre 2007, n. 770 | Delibera |
| Campania | D.G.R. 20 luglio 2001, n. 3530 e D.G.R. 17 novembre 2004, n. 2088 | Delibera |
| Emilia-Romagna | D.G.R. 18 luglio 2005, n. 1092 | Delibera |
| Friuli-Venezia Giulia | L.R. 9 gennaio 2018, n. 1 | Legge regionale |
| Lazio | Regolamento Regionale 1 dicembre 2017, n. 29 | Regolamento |
| Liguria | D.G.R. 23 luglio 2014, n. 902 | Delibera |
| Lombardia | D.G.R. 17 maggio 2006, n. 8/2552 | Delibera *(con requisiti propri sugli AB)* |
| Marche | D.G.R. 23 luglio 2012, n. 1136 | Delibera |
| Molise | L.R. 21 novembre 2008, n. 33 | Legge regionale |
| Piemonte | D.G.R. 28 aprile 2003, n. 119-9199 (+ linee guida) | Delibera |
| Puglia | L.R. 15 dicembre 2008, n. 35 *(preceduta da D.G.R. 22 giugno 2004, n. 909)* | Legge regionale |
| Sardegna | D.G.R. 5 febbraio 2019, n. 6/28 *(agg. D.G.R. 4 maggio 2023, n. 17/39)* | Delibera |
| Sicilia | *Nessun atto regionale proprio*: si applica direttamente l'Accordo 2003 + Disciplina interregionale 2004 | — (solo normativa nazionale) |
| Toscana | L.R. 9 marzo 2006, n. 8 *(+ Reg. D.P.G.R. 23/R/2010)* | Legge regionale |
| Trentino-Alto Adige — P.A. Trento | Delibera 9 marzo 2007, n. 480 | Delibera |
| Trentino-Alto Adige — P.A. Bolzano | Delibera 28 giugno 2004, n. 2360 | Delibera |
| Umbria | L.R. 13 febbraio 2007, n. 4 *(oggi T.U. L.R. 9 aprile 2015, n. 11, Capo III)* + R.R. 1 aprile 2008, n. 2 | Legge regionale |
| Valle d'Aosta | *Nessun atto regionale proprio*: l'Azienda USL applica direttamente l'Accordo 2003 + Disciplina interregionale 2004 | — (solo normativa nazionale) |
| Veneto | D.G.R. 18 aprile 2003, n. 1173 | Delibera |

!!! warning "I riferimenti possono essere superati"
    I provvedimenti regionali vengono **modificati, integrati o sostituiti** nel tempo. I riferimenti
    qui sopra sono un **punto di partenza**, non la garanzia del testo vigente: prima di usarli,
    verifica sempre sul **Bollettino Ufficiale** della tua Regione.

## Fonti utili (NON ufficiali) per partire

Questi siti raccolgono e indicizzano le norme regionali. Sono **comodi come indice**, ma non sono
fonti ufficiali (e a volte sono incompleti o non aggiornati): usali per *trovare* il provvedimento,
poi leggilo sulla fonte regionale.

- [Accordo Stato-Regioni 16/01/2003 – Gazzetta Ufficiale](https://www.gazzettaufficiale.it/eli/id/2003/03/03/03A02358/sg)
- [Assopiscine – Delibere e Leggi Regionali](https://www.assopiscine.it/norme-e-regolamenti/delibere-e-leggi-regionali/)
- [Professione Acqua – Leggi Regionali Piscine](https://www.professioneacqua.it/leggi-regionali-piscine/)

---

Conosci con certezza il provvedimento della tua Regione (con riferimento ufficiale)?
**[Aiutaci a completare la tabella →](../contribuisci.md)**
